...
namespace LA 
{
			...
			else
			{
				...
				else
				{
					if (source.text[i] == '\'')
					{
						...
						{
							if (SizeOfStringLiteral <= TI_STR_MAXSIZE)
							{
								tmp[tmpSize] = source.text[i];
								ColInString++;
							}
					...
					else if (source.text[i] == '{' && lexTable.table[lexTable.current_size - 1].lexema == LEX_DECLARATION)
					{
	...
	bool TokenAnalysis(char* token, int strNumber, LexTable& lexTable, IdTable& idTable, int colinstring)
	{
		...
		case 'i':
		{
			...
			{
				FST::FST* auto_value = new FST::FST(INTEGER(token));
				if (FST::execute(*auto_value))
				{
					LT::Add(lexTable, { LEX_INTEGER, strNumber, (int)LT_TI_NULLXDX });
					delete auto_value;
					auto_value = NULL;
		...
		case 'M':
		{
			FST::FST* auto_value = new FST::FST(MAIN(token));
			if (FST::execute(*auto_value))
			{
				LT::Add(lexTable, { LEX_MAIN, strNumber, (int)LT_TI_NULLXDX });
				delete auto_value;
				auto_value = NULL;
				return true;
			}
		...
		case 'a':
		{
			FST::FST* auto_value = new FST::FST(AS(token));
			if (FST::execute(*auto_value))
			{
				LT::Add(lexTable, { LEX_AS, strNumber, (int)LT_TI_NULLXDX });
				delete auto_value;
				auto_value = NULL;
				return true;
			}
		...
		case 'd':
		{
			FST::FST* auto_value = new FST::FST(DECLARATION(token));
			if (FST::execute(*auto_value))
			{
				LT::Add(lexTable, { LEX_DECLARATION, strNumber, (int)LT_TI_NULLXDX });
				delete auto_value;
				auto_value = NULL;
				return true;
			}
		...
		case 'l':
		{
			FST::FST* auto_value = new FST::FST(LOGIC(token));
			if (FST::execute(*auto_value))
			{
				LT::Add(lexTable, { LEX_LOGIC, strNumber, (int)LT_TI_NULLXDX });
				delete auto_value;
				auto_value = NULL;
				tov.LT_position = lexTable.current_size - 1;
				tov.type_of_variable = TypeOfVariable::LOGIC;
				return true;
			}
			else
			{
...
#pragma region IS_ARRAY
				...
				{
					if (IsId(idTable, token) == TI_NULLIDX || idTable.table[IsId(idTable, token)].idtype != V || strcmp(idTable.table[IsId(idTable, token)].parrent_block_name, idTable.table[lexTable.table[lexTable.current_size].idxTI].id) != 0)
					{
						IT::Entry entry;
						strcpy_s(entry.parrent_block_name, "main");
						...
						if (type.type_of_variable == TypeOfVariable::LOGIC)
						{
...
#pragma region IS_DECLARE_WITH_REF
				...
				{
					if (IsId(idTable, token) == TI_NULLIDX || idTable.table[IsId(idTable, token)].idtype != V || idTable.table[IsId(idTable, token)].parrent_block_name != idTable.table[lexTable.table[i].idxTI].id)
					{
						IT::Entry entry;
						strcpy_s(entry.parrent_block_name, idTable.table[lexTable.table[lexTable.current_size - 2].idxTI].id);
						strcpy_s(entry.id, token);
...
#pragma region IS_PROTO_FUNCTION
				...
				if (strcmp(token, "main") == 0 || (!is_already_checked && ((GetEntry(lexTable, lexTable.current_size - 1).lexema == LEX_FUNCTION && GetEntry(lexTable, lexTable.current_size - 2).lexema == 't') && type.LT_position == lexTable.current_size - 2)))
				{
					IT::Entry entry;
					strcpy_s(entry.parrent_block_name, "");
					strcpy_s(entry.id, token);
					...
					if (IsId(idTable, token) == -1 || idTable.table[IsId(idTable, token)].idtype != PF)
					{
						...
						{
							IT::Add(idTable, entry);
							LT::Add(lexTable, { LEX_MAIN, strNumber, idTable.current_size - 1 });
						}
						else
						{
							if (type.type_of_variable == TypeOfVariable::INT)
...
#pragma region IS_FUNCTION
				...
				if ((strcmp(token, "main") == 0 || strcmp(token, "printcurrentdate") == 0 || strcmp(token, "isvisokosni") == 0) || (!is_already_checked && ((GetEntry(lexTable, lexTable.current_size - 1).lexema == LEX_FUNCTION && GetEntry(lexTable, lexTable.current_size - 2).lexema == 't') && type.LT_position == lexTable.current_size - 2)))
				{
					IT::Entry entry;
					strcpy_s(entry.parrent_block_name, "");
					strcpy_s(entry.id, token);
					...
					if (IsId(idTable, token) == -1 || idTable.table[IsId(idTable, token)].idtype != F)
					{
						...
						else if (strcmp(token, "isvisokosni") == 0)
						{
							entry.idtype = PF;
...
#pragma region ID_WITHOUT_DECLARE
			...
			if (!is_already_checked)
			{
				if (lexTable.table[lexTable.current_size - 1].lexema != LEX_INTEGER)
				{
					...
					for (int i = lexTable.current_size - 1; i > 0; i--)
					{
						...
						if (isLeftBraceWas && ((lexTable.table[i].lexema == LEX_ID && idTable.table[lexTable.table[i].idxTI].idtype == IT::IDTYPE::F) || lexTable.table[i].lexema == LEX_MAIN))
						{
							int tempIndex = IsId(idTable, token);
							if (lexTable.table[i].lexema == LEX_MAIN)
							{
								...
								if (tempIndex != -1 && !strcmp(idTable.table[tempIndex].parrent_block_name, "main"))
								{
									LT::Add(lexTable, { LEX_ID, strNumber, tempIndex });
									break;
								}
