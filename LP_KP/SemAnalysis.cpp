...
{
	//отсутствие блока declaration
	if(lextable.table[0].lexema	!= LEX_DECLARATION)
	...
	//наличие функции main и её единственности
	bool main_was = false;
	...
	for (int i = decl_end; i < lextable.current_size; i++)
	{
		if (lextable.table[i].lexema == LEX_AS && lextable.table[i - 1].lexema == LEX_ID && idtable.table[lextable.table[i - 1].idxTI].idtype == IT::IDTYPE::P)
		{
	...
	for (int i = decl_end, str = 0; i < lextable.current_size; i++)
	{
		...
		if (lextable.table[i].lexema == LEX_AS && lextable.table[i - 1].lexema == LEX_ID)
		{
			int id_pos = i - 1;
	...
	for (int i = decl_end, str = 0; i < lextable.current_size; i++)
	{
		...
		{
			int j = i;
			while (lextable.table[j].lexema != LEX_FUNCTION && lextable.table[j].lexema != LEX_MAIN)
				j--;
			if (lextable.table[j].lexema == LEX_MAIN)
				break;
