...
void code_block_generation(std::ofstream* file, int& i, LT::LexTable& LexTable, IT::IdTable& IdTable)
{
	...
	{
		if (LexTable.table[i].lexema == LEX_AS)
		{
			int indOflex = i-1;
			...
			while (LexTable.table[i].lexema != LEX_SEMICOLON)
			{
				...
				else if (LexTable.table[i].lexema == LEX_ID)
				{
					if (IdTable.table[LexTable.table[i].idxTI].idtype == IT::IDTYPE::PF)
					{
						...
						if(strcmp(IdTable.table[LexTable.table[function_id].idxTI].id, "isvisokosni") != 0  && strcmp(IdTable.table[LexTable.table[function_id].idxTI].id, "printcurrentdate") != 0)
...
{
	std::cout << std::endl;
	std::ofstream* file = new std::ofstream("D:\\Нада\\ЯП\\KP\\IMS_2020asm\\IMS_2020asm\\IMS_2020asm.asm");
	if (file->is_open())
	{
		*file << STANDART_BEGIN;
		*file << STANDART_LIB;
		...
		*file << LITERALS_BEGIN;
		...
		for (int i = 0; i < IdTable.current_size; i++)
		{
			if (IdTable.table[i].idtype == IT::IDTYPE::L)
			{
				...
				{
					*file << 'L' << i << "\tsdword\t" << IdTable.table[i].value.vint.value << "\t ; integer literal" << '\n';
					break;
				}
		...
		for (int i = 0; i < IdTable.current_size; i++)
		{
			if (IdTable.table[i].idtype == IT::IDTYPE::V)
			{
				*file  << IdTable.table[i].id;
				...
				case IT::IDDATATYPE::INT:  
				{
					*file << "\tsdword\t" << '0' << "\t ; integer variable" << '\n';
					break;
				}

				case IT::IDDATATYPE::STR: 
				{
					*file << "\tbyte\t" << TI_STR_MAXSIZE << " dup(0)" << "\t ; string variable" << '\n';
					break;
				}

				case IT::IDDATATYPE::LOG: 
				{
					*file << "\tsdword\t" << '0' << "\t ; logic variable" << '\n'; 
					break;
				}
				}
			}			
