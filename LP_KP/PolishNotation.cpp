...
void PolishNotation(LT::LexTable& lextable, IT::IdTable& idtable)
{
	...
	{
		if (lextable.table[i].lexema == LEX_AS || lextable.table[i].lexema == LEX_RETURN || lextable.table[i].lexema == LEX_PRINT)
		{
			int parm_counter = 0;
			...
			while (lextable.table[i].lexema != LEX_SEMICOLON)
			{
				...
				if (lextable.table[i].lexema == LEX_AS || lextable.table[i].lexema == LEX_RIGHT_SQ_BRACE || lextable.table[i].lexema == LEX_LEFT_SQ_BRACE || lextable.table[i].lexema == LEX_RETURN || lextable.table[i].lexema == LEX_PRINT)
				{
					LT::Add(NewLexTable, { lextable.table[i++].lexema, lextable.table[i].sn, lextable.table[i].idxTI });
					continue;
				}
				
				if (lextable.table[i].lexema == LEX_PLUS && ((lextable.table[i - 1].lexema == LEX_LITERAL && idtable.table[lextable.table[i - 1].idxTI].iddatatype == IT::IDDATATYPE::STR) || (lextable.table[i - 1].lexema == LEX_ID && idtable.table[lextable.table[i - 1].idxTI].iddatatype == IT::IDDATATYPE::STR)))
				{
